...
int main(int argc, char *argv[])
{
	...
	if(fd1 != NULL){
		...
		fprintf(fd1, "round argv[6]= %s \n", argv[6]); /* round */
		fprintf(fd1, "cc argv[7]= %s \n", argv[7]); /* cc */
		#endif
	}
	...
	char reportliacmd[100] = "";
								      //$5 $6 $7 $1 t         exp      round    cc       client   time
	sprintf(reportliacmd, "mv iperf-result.txt /home/chix/nfs-share/%s/%s/%s-%s-%s.txt",  argv[5], argv[6], argv[7], argv[1], time_stamp() );
