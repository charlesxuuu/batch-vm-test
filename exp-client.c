...
int main(int argc, char *argv[])
 {
	...
	mycall("sysctl net.ipv4.tcp_congestion_control='cubic' >> log.txt");
	...
	#endif
	fprintf(fd1, "iperf-cubic-lines: %d \n", countline("iperf-result.txt"));
	mycall(reportcubiccmd);

	mycall("sleep 20");

	/* dctcp */
	mycall("sysctl net.ipv4.tcp_congestion_control='dctcp' >> log.txt");
	...
	mycall("head -n -1 iperf-result.tmp > iperf-result.txt");
	#endif
	fprintf(fd1, "iperf-dctcp-lines: %d \n", countline("iperf-result.txt"));
	mycall(reportdctcpcmd);
	...
	/* my */
	mycall("insmod /home/chix/mptcp_ecn.ko >> log.txt");
